...
def test_mine_filter_returns_only_own_brews(
    client, alice_headers, bob_headers, alice_stack, lookups
):
    bob_brew_id = client.post(
        "/brews",
        headers=bob_headers,
...
def test_non_author_cannot_edit_or_delete_tasting(client, bob_headers, alice_stack):
    ...
    assert (
        client.patch(f"/tastings/{tid}", headers=bob_headers, json={"overall": 3}).status_code
        == 403
    )
...
def test_delete_bean_cascades(client, alice_headers, alice_stack):
    ...
    assert (
        client.get(f"/tastings/{alice_stack['tasting']}", headers=alice_headers).status_code == 404
    )
