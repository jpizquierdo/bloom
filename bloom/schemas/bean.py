...
class BeanBase(BaseModel):
    origin_country: str | None = Field(
        default=None, description="Country of origin.", examples=["Ethiopia"]
    )
    region: str | None = Field(default=None, description="Growing region.", examples=["Guji"])
    producer: str | None = Field(
        default=None, description="Farm or producer.", examples=["Tabe Burka"]
    )
    variety: str | None = Field(default=None, description="Coffee variety.", examples=["Heirloom"])
    process: Process | None = Field(
        default=None, description="Post-harvest process.", examples=["washed"]
    )
    roast_level: RoastLevel | None = Field(
        default=None, description="Roast level.", examples=["medium_light"]
    )
    roast_date: date | None = Field(
        default=None, description="Roast date.", examples=["2026-07-01"]
    )
    purchase_date: date | None = Field(
        default=None, description="Purchase date.", examples=["2026-07-05"]
    )
    weight_grams: int | None = Field(
        default=None, gt=0, description="Bag weight in grams.", examples=[250]
    )
    price: Decimal | None = Field(
        default=None, ge=0, description="Price paid for the bag.", examples=["18.50"]
    )
    altitude_masl: int | None = Field(
        default=None, description="Growing altitude (metres above sea level).", examples=[2100]
    )
    ...
    notes: str | None = Field(
        default=None, description="Your own notes.", examples=["Great as filter"]
    )
...
class BeanCreate(BeanBase):
    ...
    is_finished: bool = Field(
        default=False, description="Whether the bag is used up.", examples=[False]
    )
...
class BeanUpdate(BeanBase):
    ...
    name: str | None = Field(
        default=None, min_length=1, description="Coffee name.", examples=["Guji Natural"]
    )
    roaster: str | None = Field(
        default=None, min_length=1, description="Roaster.", examples=["Nomad Coffee"]
    )
    is_finished: bool | None = Field(
        default=None, description="Whether the bag is used up.", examples=[True]
    )
