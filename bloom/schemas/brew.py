...
class BrewBase(BaseModel):
    grinder_id: int | None = Field(
        default=None, description="Grinder used (equipment id).", examples=[1]
    )
    ...
    yield_grams: Decimal | None = Field(
        default=None, gt=0, description="Beverage mass in the cup (g).", examples=["250"]
    )
    water_grams: Decimal | None = Field(
        default=None, gt=0, description="Water used (g); for filter/immersion.", examples=["250"]
    )
    grind_setting: str | None = Field(
        default=None, description="Grinder setting (grinder-specific).", examples=["18"]
    )
    water_temp_celsius: Decimal | None = Field(
        default=None, description="Water temperature (°C).", examples=["93.0"]
    )
    brew_time_seconds: int | None = Field(
        default=None, gt=0, description="Total brew time (s).", examples=[150]
    )
    tds_percent: Decimal | None = Field(
        default=None, ge=0, description="Measured TDS % (refractometer).", examples=["1.35"]
    )
    ...
    notes: str | None = Field(
        default=None, description="Free-form notes.", examples=["Even extraction"]
    )
...
class BrewUpdate(BrewBase):
    ...
    dose_grams: Decimal | None = Field(
        default=None, gt=0, description="Dry coffee dose (g).", examples=["15"]
    )
...
class ExtractionDiagnosticsRead(BaseModel):
    ...
    strength: str | None = Field(
        description="TDS band: below / within / above target.", examples=["within"]
    )
    extraction: str | None = Field(
        description="Extraction-yield band: below / within / above target.", examples=["above"]
    )
...
class BrewRead(BrewBase):
    ...
    ratio: Decimal | None = Field(
        default=None, description="Computed brew ratio (never stored).", examples=["16.67"]
    )
