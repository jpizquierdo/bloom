...
def list_tastings(brew_id: int, db: DbSession, user: CurrentUser) -> list[TastingRead]:
    return tasting_service.list_for_brew(db, brew_id, user)


@router.get("/tastings/{tasting_id}", response_model=TastingRead)
def get_tasting(tasting_id: int, db: DbSession, user: CurrentUser) -> TastingRead:
    return tasting_service.get_tasting(db, tasting_id, user)
...
def update_tasting(
    ...
    tasting = tasting_service.get_tasting(db, tasting_id, user)
